...
class rlu_trader_tick_n_size(QWidget):
    def __init__(self, login_handler):
        ...
        if my_target_list_file.is_file():
            ...
            self.rlu_ui.lineEdit_default_file_path.setText(my_target_path)
        ...
        self.rlu_ui.folder_btn.clicked.connect(self.showDialog)
        self.rlu_ui.read_csv_btn.clicked.connect(self.read_target_list)
        self.rlu_ui.button_start.clicked.connect(self.on_button_start_clicked)
        self.rlu_ui.button_stop.clicked.connect(self.on_button_stop_clicked)
        self.rlu_ui.button_fake_buy_filled.clicked.connect(self.fake_buy_filled)
        self.rlu_ui.button_fake_websocket.clicked.connect(self.fake_ws_data)
    ...
    def fake_message(self, stock_no):
        ...
        if json_price >= 20:
            json_template = '''{{"event":"data","data":{{"symbol":"{symbol}","type":"EQUITY","exchange":"TWSE","market":"TSE","price":{price},"size":199,"bid":16.67,"ask":{price}, "isLimitUpPrice":true, "volume":500,"isClose":true,"time":1718343000000000,"serial":9475857}},"id":"w4mkzAqYAYFKyEBLyEjmHEoNADpwKjUJmqg02G3OC9YmV","channel":"trades"}}'''
    ...
    def update_table_row(self, symbol, price, bid, ask, is_limit_up):
        if bid > 0:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['買進']).setText(str(round(bid+self.epsilon, 2)))
        elif bid == 0:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['買進']).setText('市價')
        elif bid == -1:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['買進']).setText('-')

        if ask > 0:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['賣出']).setText(str(round(ask+self.epsilon, 2)))
        elif ask == 0:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['賣出']).setText('市價')
        elif ask == -1:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['賣出']).setText('-')
        
        if price>0:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['成交']).setText(str(round(price+self.epsilon, 2)))
            up_range = (price-self.last_close_dict[symbol])/self.last_close_dict[symbol]*100
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['漲幅(%)']).setText(str(round(up_range+self.epsilon, 2))+'%')
        else:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['成交']).setText('-')
        
        if is_limit_up:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['漲幅(%)']).setBackground(QColor(Qt.red))
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['漲幅(%)']).setForeground(QColor(Qt.white))
        else:
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['漲幅(%)']).setBackground(QColor(Qt.transparent))
            self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['漲幅(%)']).setForeground(QColor(Qt.black))

    def update_filled_qty_item(self, symbol, filled_qty):
        self.rlu_ui.tablewidget.item(self.row_idx_map[symbol], self.col_idx_map['成交數量']).setText(str(filled_qty))
    ...
    def on_button_start_clicked(self):
        ...
        try:
            self.trade_budget = float(self.rlu_ui.lineEdit_trade_budget.text())
        ...
        try:
            self.total_budget = float(self.rlu_ui.lineEdit_total_budget.text())
        ...
        try:
            self.volume_threshold = int(self.rlu_ui.lineEdit_total_volume.text())
        ...
        try:
            self.size_threshold = int(self.rlu_ui.lineEdit_tick_size.text())
        ...
        self.rlu_ui.read_csv_btn.setEnabled(False)
        # self.rlu_ui.tablewidget.clearContents()
        # self.rlu_ui.tablewidget.setRowCount(0)
    ...
    def on_button_stop_clicked(self):
        ...
        self.rlu_ui.lineEdit_trade_budget.setReadOnly(False)
        ...
        self.rlu_ui.read_csv_btn.setEnabled(True)
    ...
    def read_target_list(self):
        ...
        target_path = Path(self.rlu_ui.lineEdit_default_file_path.text())
        # print(target_path)
        if str(target_path) == '.':
            ...
            for symbol in new_target_symbols:
                ...
                for j in range(len(self.table_header)):
                    if self.table_header[j] == '股票名稱':
                        ...
                        self.rlu_ui.tablewidget.setItem(row, j, item)
    ...
    def showDialog(self):
        ...
        if file_path:
            self.rlu_ui.lineEdit_default_file_path.setText(file_path)
            temp_dict = {
    ...
    def print_log(self, log_info):
        self.rlu_ui.log_text.appendPlainText(log_info)
        self.rlu_ui.log_text.moveCursor(QTextCursor.End)
...
if __name__ == '__main__':
    ...
    rlu_trader = rlu_trader_tick_n_size(login_form)
    rlu_trader.show()
    rlu_trader_res = app.exec()
    sys.exit(rlu_trader_res)